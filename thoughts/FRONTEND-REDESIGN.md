...
| Rolling average gray line | Replace the misaligned overall line with a flat overall-average reference band. |
...
- Replace the per-panel gray rolling *line* with a flat reference: the overall
  average rating for the current filter set, drawn as a horizontal line or thin band.
  The current gray line plots "nth watch overall" against "nth watch in group" —
...
- DEFERRED: film-age stripes ("archive reach") — one stripe per watch colored by
