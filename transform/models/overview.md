...
history: 799 viewings of 676 films.
...
- **Ratings use two scales.** `rating_100` is 0–100 and is the authoritative one.
  ...
  factor of exactly 20 is measured across the 670 rows that arrived carrying
  both values, not assumed.
- **"Rewatches" and "returns" are different counts.** 210 rows are flagged
