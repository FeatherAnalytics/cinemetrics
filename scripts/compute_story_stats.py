...
def main() -> None:
    ...
    first_with_rating["primary_genre"] = first_with_rating["genres"].apply(_primary_genre)

    def kw_test(column: str) -> dict:
        groups = [g["rating_100"].values for _, g in first_with_rating.groupby(column)]
    ...
    by_month = rows.groupby("watch_month").size()
    oct_watches = int(by_month.get(10, 0))
    nov_watches = int(by_month.get(11, 0))
    oct_nov_ratio = round(oct_watches / max(nov_watches, 1), 1)

    by_year = rows.groupby("watch_year").size()
    pandemic = int(by_year.get(2020, 0)) + int(by_year.get(2021, 0))
    latest_full_year = int(max(y for y in by_year.index if y < rows["watch_year"].max()))
    latest_watches = int(by_year.get(latest_full_year, 0))
    pandemic_ratio = round(pandemic / max(latest_watches, 1), 1)

    answerable = rows[(rows["liked"].notna()) | (rows["is_return"])]
    years_with_enough = answerable.groupby("watch_year").size()
    valid_years = [y for y in years_with_enough.index if years_with_enough[y] >= 10]
    if len(valid_years) >= 2:
        ...
        rewatch_first = round(float(fy["is_rewatch_effective"].mean()), 3)
        rewatch_last = round(float(ly["is_rewatch_effective"].mean()), 3)
    else:
